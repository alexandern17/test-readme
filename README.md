...
The SDK can be initialized with empty strings `""` or `nil` parameters in order to run the SDK even without valid parameters, as shown below:
...
```
*Note:*
- In this state, the SDK locally queues end-user data and will upload data to the server once SDK is initialized with valid parameters.

For additional documentation and to view the sample app, see [here](https://github.com/thunderheadone/one-sdk-ios/tree/master/examples/dynamic-initialization-example).

### Reconfigurating the ONE SDK

If necessary, when the SDK is already initialized with valid parameters, you can reconfigure the SDK using new parameters at any point using the same initialization method, as shown below: 
